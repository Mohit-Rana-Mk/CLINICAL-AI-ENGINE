...
class TranslationRouter:
    ...
    def __init__(self):
        try:
            ...
            logger.exception("Language detector unavailable; using English fallback")
            self.detector = None
        try:
            from app.translation.translator import LocalTranslator
            self.translator = LocalTranslator()
        except Exception:
            logger.exception("Translation model unavailable; preserving source text")
    ...
    def process(self, text: str) -> dict:
        ...
        detection = (
            ...
            else {"language": "en", "confidence": 0.0}
        )
        ...
        if lang_code == "en":
            ...
            english_text = (
                self.translator.translate(text, source_lang_code=lang_code)
    ...
    def _process_mixed_language(self, text: str) -> str:
        ...
        try:
            ...
            for word in words:
                det = (
                    self.detector.detect_language(word)
                    if self.detector is not None
                    else {"language": "en"}
                )
                if det.get("language") in ["hi", "ur", "mr"] or word.lower() in ["mujhe", "hai", "dard", "bukhar", "aur", "khansi", "se"]:
                    ...
                    eng_word = self.translator.translate(devanagari, source_lang_code="hi") if self.translator else word
